...
const navigateToEventsPage = async (page: Page): Promise<void> => {
  try {
...
const scrapeEvents = async (page: Page): Promise<EventData[]> => {
  try {
    await page.goto("https://themetphilly.com/events", {
    ...
    const events: EventData[] = await Promise.all(
      eventContainers.map(async (container) => {
...
export const scrapeTheMetPhillyEvents = async (): Promise<EventData[]> => {
